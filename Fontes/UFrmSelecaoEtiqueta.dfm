object FormSelecaoEtiqueta: TFormSelecaoEtiqueta
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = 'FormSelecaoEtiqueta'
  ClientHeight = 503
  ClientWidth = 631
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object DBGrdMembros: TDBGrid
    Left = 0
    Top = 120
    Width = 631
    Height = 334
    Align = alClient
    DataSource = DSSelecao
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    OnCellClick = DBGrdMembrosCellClick
    OnDrawColumnCell = DBGrdMembrosDrawColumnCell
    Columns = <
      item
        Expanded = False
        FieldName = 'Selecao'
        Title.Caption = '  X'
        Width = 19
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NomeMembro'
        ReadOnly = True
        Title.Caption = 'Nome'
        Width = 558
        Visible = True
      end>
  end
  object Panel2: TPanel
    Left = 0
    Top = 0
    Width = 631
    Height = 41
    Align = alTop
    Caption = 'SELE'#199#195'O DE MEMBRO PARA IMPRESS'#195'O'
    Color = clHotLight
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindow
    Font.Height = -21
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentBackground = False
    ParentFont = False
    TabOrder = 1
  end
  object ToolBar1: TToolBar
    Left = 0
    Top = 41
    Width = 631
    Height = 52
    ButtonHeight = 53
    Caption = 'ToolBar1'
    TabOrder = 2
    object BitBtn3: TBitBtn
      Left = 0
      Top = 0
      Width = 233
      Height = 53
      Caption = 'SELECIONAR TUDO'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 2
      OnClick = BitBtn3Click
    end
    object BitBtn2: TBitBtn
      Left = 233
      Top = 0
      Width = 212
      Height = 53
      Caption = 'DESMARCAR TODOS'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      OnClick = BitBtn2Click
    end
    object BitBtn1: TBitBtn
      Left = 445
      Top = 0
      Width = 184
      Height = 53
      Caption = 'INVERTER SELE'#199#195'O'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      OnClick = BitBtn1Click
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 454
    Width = 631
    Height = 49
    Align = alBottom
    Color = clHotLight
    ParentBackground = False
    TabOrder = 3
    object BtnSalvar: TBitBtn
      Left = 233
      Top = 6
      Width = 75
      Height = 41
      Caption = '&Imprimir'
      Default = True
      Glyph.Data = {
        
        4CFEC4763B94FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00}
      Layout = blGlyphTop
      TabOrder = 0
      OnClick = BtnSalvarClick
    end
    object BtnCancelar: TBitBtn
      Left = 328
      Top = 6
      Width = 75
      Height = 41
      Caption = 'Sai&r'
      Glyph.Data = {
        
        FF00FFFFFF00FFFFFF00FFFFFF006D9CD4896A9AD2FB6697CFEE}
      Layout = blGlyphTop
      TabOrder = 1
      OnClick = BtnCancelarClick
    end
  end
  object EdtBusca: TEdit
    Left = 0
    Top = 93
    Width = 631
    Height = 27
    Align = alTop
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -16
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 4
    Text = 'Digite o nome para buscar...'
    OnChange = EdtBuscaChange
    OnClick = EdtBuscaClick
  end
  object CDSSelecao: TClientDataSet
    Active = True
    Aggregates = <>
    FieldDefs = <
      item
        Name = 'CodMembro'
        DataType = ftInteger
      end
      item
        Name = 'Selecao'
        DataType = ftBoolean
      end
      item
        Name = 'NomeMembro'
        DataType = ftString
        Size = 255
      end>
    IndexDefs = <>
    Params = <>
    StoreDefs = True
    Left = 576
    Top = 224
    Data = {
      5B0000009619E0BD0100000018000000030000000000030000005B0009436F64
      4D656D62726F04000100000000000753656C6563616F02000300000000000A4E
      6F6D654D656D62726F020049000000010005574944544802000200FF000000}
    object CDSSelecaoCodMembro: TIntegerField
      FieldName = 'CodMembro'
    end
    object CDSSelecaoSelecao: TBooleanField
      FieldName = 'Selecao'
    end
    object CDSSelecaoNomeMembro: TStringField
      FieldName = 'NomeMembro'
      Size = 255
    end
  end
  object DSSelecao: TDataSource
    DataSet = CDSSelecao
    Left = 576
    Top = 264
  end
end
