object FormCadastroMembroFilho: TFormCadastroMembroFilho
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = 'FILHOS'
  ClientHeight = 174
  ClientWidth = 527
  Color = clWindow
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  ShowHint = True
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel2: TPanel
    Left = 0
    Top = 0
    Width = 527
    Height = 49
    Align = alTop
    Caption = 'FILHOS'
    Color = clHotLight
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindow
    Font.Height = -21
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentBackground = False
    ParentFont = False
    TabOrder = 0
    ExplicitWidth = 512
  end
  object Panel3: TPanel
    Left = 0
    Top = 125
    Width = 527
    Height = 49
    Align = alBottom
    Color = clHotLight
    ParentBackground = False
    TabOrder = 1
    ExplicitWidth = 512
    object BtnSalvar: TBitBtn
      Left = 345
      Top = 6
      Width = 75
      Height = 41
      Caption = '&Salvar'
      Default = True
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      TabOrder = 0
      OnClick = BtnSalvarClick
    end
    object BtnCancelar: TBitBtn
      Left = 426
      Top = 6
      Width = 75
      Height = 41
      Caption = '&Cancelar'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      TabOrder = 1
      OnClick = BtnCancelarClick
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 49
    Width = 527
    Height = 76
    Align = alClient
    Caption = '``'
    Color = clInactiveCaption
    ParentBackground = False
    TabOrder = 2
    ExplicitWidth = 512
    object Label2: TLabel
      Left = 12
      Top = 12
      Width = 35
      Height = 13
      Caption = 'Nome:'
      FocusControl = DBEdtNomeFilho
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label3: TLabel
      Left = 240
      Top = 14
      Width = 116
      Height = 13
      Caption = 'Data de Nascimento:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object DBEdtNomeFilho: TcxDBTextEdit
      Left = 12
      Top = 31
      DataBinding.DataField = 'NomeFilho'
      DataBinding.DataSource = FormCadastroMembro.DSFilhos
      TabOrder = 0
      Width = 221
    end
    object DBDtEdtDataNascimento: TDBDateEdit
      Left = 240
      Top = 31
      Width = 121
      Height = 21
      Margins.Left = 4
      Margins.Top = 1
      DataField = 'DataNasc'
      DataSource = FormCadastroMembro.DSFilhos
      NumGlyphs = 2
      TabOrder = 1
    end
    object DBRdGrpFiliadoCongregacao: TDBRadioGroup
      Left = 371
      Top = 14
      Width = 134
      Height = 39
      Caption = ' Filiado a congrega'#231#227'o? '
      Columns = 2
      DataField = 'IndComunidade'
      DataSource = FormCadastroMembro.DSFilhos
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Items.Strings = (
        'N'#195'O'
        'SIM')
      ParentBackground = True
      ParentFont = False
      TabOrder = 2
      Values.Strings = (
        '0'
        '1')
    end
  end
end
