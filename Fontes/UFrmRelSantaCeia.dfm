object FormRelSantaCeia: TFormRelSantaCeia
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = 'RELAT'#211'RIO DE SANTA CEIA'
  ClientHeight = 295
  ClientWidth = 446
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel2: TPanel
    Left = 0
    Top = 0
    Width = 446
    Height = 41
    Align = alTop
    Caption = 'RELAT'#211'RIO DE SANTA CEIA'
    Color = clHotLight
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindow
    Font.Height = -21
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentBackground = False
    ParentFont = False
    TabOrder = 0
  end
  object Panel3: TPanel
    Left = 0
    Top = 246
    Width = 446
    Height = 49
    Align = alBottom
    Color = clHotLight
    ParentBackground = False
    TabOrder = 1
    object BtnSalvar: TBitBtn
      Left = 150
      Top = 6
      Width = 75
      Height = 41
      Caption = '&Imprimir'
      Default = True
      Glyph.Data = {
        
        4CFEC4763B94FFFFFF00FFFFFF00FFFFFF00FFFFFF00FFFFFF00}
      Layout = blGlyphTop
      TabOrder = 0
      OnClick = BtnSalvarClick
    end
    object BtnCancelar: TBitBtn
      Left = 231
      Top = 6
      Width = 75
      Height = 41
      Caption = 'Sai&r'
      Glyph.Data = {
        
        FF00FFFFFF00FFFFFF00FFFFFF006D9CD4896A9AD2FB6697CFEE}
      Layout = blGlyphTop
      TabOrder = 1
      OnClick = BtnCancelarClick
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 41
    Width = 446
    Height = 205
    Align = alClient
    Color = clInactiveCaption
    ParentBackground = False
    TabOrder = 2
    object Label1: TLabel
      Left = 61
      Top = 12
      Width = 50
      Height = 13
      Caption = 'Membro:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label2: TLabel
      Left = 68
      Top = 39
      Width = 43
      Height = 13
      Caption = 'Fam'#237'lia:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Labelde: TLabel
      Left = 120
      Top = 151
      Width = 13
      Height = 13
      Caption = 'DE'
      Visible = False
    end
    object LabelATE: TLabel
      Left = 247
      Top = 151
      Width = 23
      Height = 13
      Caption = 'AT'#201':'
      Visible = False
    end
    object Label3: TLabel
      Left = 84
      Top = 66
      Width = 27
      Height = 13
      Caption = 'Tipo:'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object CbxMembros: TComboBox
      Left = 117
      Top = 9
      Width = 251
      Height = 21
      Style = csDropDownList
      ItemIndex = 0
      TabOrder = 0
      Text = 'TODAS'
      Items.Strings = (
        'TODAS')
    end
    object CbxFamilia: TComboBox
      Left = 117
      Top = 36
      Width = 251
      Height = 21
      Style = csDropDownList
      ItemIndex = 0
      TabOrder = 1
      Text = 'TODAS'
      Items.Strings = (
        'TODAS'
        'PAGO'
        'DEVEDOR')
    end
    object RdGrpData: TRadioGroup
      Left = 117
      Top = 92
      Width = 251
      Height = 57
      Caption = 'Selecione o Periodo: '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ItemIndex = 0
      Items.Strings = (
        'TODAS'
        'SELECIONAR INTERVALO')
      ParentFont = False
      TabOrder = 3
      OnClick = RdGrpDataClick
    end
    object DtEdtDE: TcxDateEdit
      Left = 117
      Top = 167
      TabOrder = 4
      Visible = False
      Width = 121
    end
    object DtEdtATE: TcxDateEdit
      Left = 247
      Top = 167
      TabOrder = 5
      Visible = False
      Width = 121
    end
    object CbxTipo: TComboBox
      Left = 117
      Top = 63
      Width = 251
      Height = 21
      Style = csDropDownList
      ItemIndex = 0
      TabOrder = 2
      Text = 'TODAS'
      Items.Strings = (
        'TODAS'
        'MEMBROS'
        'N'#195'O MEMBROS')
    end
  end
  object QryTemp: TMyQuery
    Connection = DMPrincipal.Conn
    Left = 388
    Top = 8
  end
  object QryRel: TMyQuery
    Connection = DMPrincipal.Conn
    SQL.Strings = (
      
        'SELECT M.NomeMembro, F.Sobrenome, s.*, (Janeiro + Fevereiro + Ma' +
        'rco + Abril + Maio + Junho + Julho + Agosto + Setembro + Outubro' +
        ' + Novembro + Dezembro) as TotalAnual'
      
        'FROM santa_ceia_antigo S left join Membros M on S.CodMembro = M.' +
        'CodMembro'
      
        '                         left join Familia F on S.CodFamilia = F' +
        '.CodFamilia'
      
        'Order by S.CodFamilia, F.Sobrenome collate latin1_german1_ci, S.' +
        'CodMembroAntigo, S.Ano')
    Left = 28
    Top = 8
    object QryRelNomeMembro: TStringField
      FieldName = 'NomeMembro'
      Origin = 'M.NomeMembro'
      Size = 255
    end
    object QryRelSobrenome: TStringField
      FieldName = 'Sobrenome'
      Origin = 'F.Sobrenome'
      Size = 255
    end
    object QryRelCodSantaCeia: TLargeintField
      FieldName = 'CodSantaCeia'
      Origin = 'S.CodSantaCeia'
    end
    object QryRelCodMembro: TLargeintField
      FieldName = 'CodMembro'
      Origin = 'S.CodMembro'
    end
    object QryRelCodMembroAntigo: TLargeintField
      FieldName = 'CodMembroAntigo'
      Origin = 'S.CodMembroAntigo'
    end
    object QryRelCodFamilia: TLargeintField
      FieldName = 'CodFamilia'
      Origin = 'S.CodFamilia'
    end
    object QryRelAno: TLargeintField
      FieldName = 'Ano'
      Origin = 'S.Ano'
    end
    object QryReljaneiro: TLargeintField
      FieldName = 'janeiro'
      Origin = 'S.janeiro'
    end
    object QryRelfevereiro: TLargeintField
      FieldName = 'fevereiro'
      Origin = 'S.fevereiro'
    end
    object QryRelmarco: TLargeintField
      FieldName = 'marco'
      Origin = 'S.marco'
    end
    object QryRelabril: TLargeintField
      FieldName = 'abril'
      Origin = 'S.abril'
    end
    object QryRelmaio: TLargeintField
      FieldName = 'maio'
      Origin = 'S.maio'
    end
    object QryReljunho: TLargeintField
      FieldName = 'junho'
      Origin = 'S.junho'
    end
    object QryReljulho: TLargeintField
      FieldName = 'julho'
      Origin = 'S.julho'
    end
    object QryRelagosto: TLargeintField
      FieldName = 'agosto'
      Origin = 'S.agosto'
    end
    object QryRelsetembro: TLargeintField
      FieldName = 'setembro'
      Origin = 'S.setembro'
    end
    object QryReloutubro: TLargeintField
      FieldName = 'outubro'
      Origin = 'S.outubro'
    end
    object QryRelnovembro: TLargeintField
      FieldName = 'novembro'
      Origin = 'S.novembro'
    end
    object QryReldezembro: TLargeintField
      FieldName = 'dezembro'
      Origin = 'S.dezembro'
    end
    object QryRelTotalAnual: TLargeintField
      FieldName = 'TotalAnual'
      Origin = 'TotalAnual'
    end
  end
  object DSRel: TDataSource
    DataSet = QryRel
    Left = 28
    Top = 56
  end
  object frxRpt: TfrxReport
    Version = '4.12.2'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 41526.672388125000000000
    ReportOptions.LastChange = 41526.672388125000000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 28
    Top = 104
    Datasets = <
      item
        DataSet = frxDBDataset
        DataSetName = 'frxDBDataset1'
      end>
    Variables = <
      item
        Name = ' Geral'
        Value = Null
      end
      item
        Name = 'TotalFamilias'
        Value = ''
      end
      item
        Name = 'TotalMembros'
        Value = ''
      end
      item
        Name = 'DataInicio'
        Value = ''
      end
      item
        Name = 'DataFim'
        Value = ''
      end
      item
        Name = 'Familia'
        Value = ''
      end
      item
        Name = 'Tipo'
        Value = ''
      end>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      Orientation = poLandscape
      PaperWidth = 297.000000000000000000
      PaperHeight = 210.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object PageHeader1: TfrxPageHeader
        Height = 86.929190000000000000
        Top = 18.897650000000000000
        Width = 1046.929810000000000000
        object SysMemo1: TfrxSysMemoView
          Left = 925.984850000000000000
          Top = 11.338590000000000000
          Width = 113.385900000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Memo.UTF8W = (
            'Data: [DATE]')
        end
        object SysMemo2: TfrxSysMemoView
          Left = 925.984850000000000000
          Top = 34.015770000000000000
          Width = 113.385900000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Memo.UTF8W = (
            'P'#225'gina: [PAGE#]')
        end
        object Memo8: TfrxMemoView
          Top = 11.338590000000000000
          Width = 1046.929810000000000000
          Height = 30.236240000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -24
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            'ADMINISTRA'#199#195'O DE IGREJA')
          ParentFont = False
        end
        object Memo9: TfrxMemoView
          Top = 41.574830000000000000
          Width = 279.685220000000000000
          Height = 26.456710000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'Relat'#243'rio de Santa Ceia')
          ParentFont = False
        end
        object Line2: TfrxLineView
          Top = 60.472480000000000000
          Width = 1046.929810000000000000
          ShowHint = False
          Frame.Typ = [ftTop]
        end
        object DataInicio: TfrxMemoView
          Top = 60.472480000000000000
          Width = 1046.929810000000000000
          Height = 26.456710000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            
              'Periodo: [DataInicio] at'#233' [DataFim]      Familia: [Familia]     ' +
              ' Tipo: [Tipo]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object MasterData1: TfrxMasterData
        Height = 18.897650000000000000
        Top = 234.330860000000000000
        Width = 1046.929810000000000000
        DataSet = frxDBDataset
        DataSetName = 'frxDBDataset1'
        RowCount = 0
        object frxDBDataset1CodMembro: TfrxMemoView
          Width = 86.929190000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataSet = frxDBDataset
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8W = (
            '   [frxDBDataset1."CodMembro"]')
          ParentFont = False
        end
        object frxDBDataset1NomeMembro: TfrxMemoView
          Left = 86.929190000000000000
          Width = 423.307360000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataSet = frxDBDataset
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8W = (
            ' [frxDBDataset1."NomeMembro"]')
          ParentFont = False
        end
        object frxDBDataset1Ano: TfrxMemoView
          Left = 510.236550000000000000
          Width = 45.354360000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'Ano'
          DataSet = frxDBDataset
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDataset1."Ano"]')
          ParentFont = False
        end
        object frxDBDataset1janeiro: TfrxMemoView
          Left = 555.590910000000000000
          Width = 34.015770000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'janeiro'
          DataSet = frxDBDataset
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDataset1."janeiro"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxDBDataset1fevereiro: TfrxMemoView
          Left = 589.606680000000000000
          Width = 34.015770000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'fevereiro'
          DataSet = frxDBDataset
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDataset1."fevereiro"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxDBDataset1marco: TfrxMemoView
          Left = 623.622450000000000000
          Width = 34.015770000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'marco'
          DataSet = frxDBDataset
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDataset1."marco"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxDBDataset1abril: TfrxMemoView
          Left = 657.638220000000000000
          Width = 34.015770000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'abril'
          DataSet = frxDBDataset
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDataset1."abril"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxDBDataset1maio: TfrxMemoView
          Left = 691.653990000000000000
          Width = 34.015770000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'maio'
          DataSet = frxDBDataset
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDataset1."maio"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxDBDataset1junho: TfrxMemoView
          Left = 725.669760000000000000
          Width = 34.015770000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'junho'
          DataSet = frxDBDataset
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDataset1."junho"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxDBDataset1julho: TfrxMemoView
          Left = 759.685530000000000000
          Width = 34.015770000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'julho'
          DataSet = frxDBDataset
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDataset1."julho"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxDBDataset1agosto: TfrxMemoView
          Left = 793.701300000000000000
          Width = 34.015770000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'agosto'
          DataSet = frxDBDataset
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDataset1."agosto"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxDBDataset1setembro: TfrxMemoView
          Left = 827.717070000000000000
          Width = 34.015770000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'setembro'
          DataSet = frxDBDataset
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDataset1."setembro"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxDBDataset1outubro: TfrxMemoView
          Left = 861.732840000000000000
          Width = 34.015770000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'outubro'
          DataSet = frxDBDataset
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDataset1."outubro"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxDBDataset1novembro: TfrxMemoView
          Left = 895.748610000000000000
          Width = 34.015770000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'novembro'
          DataSet = frxDBDataset
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDataset1."novembro"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxDBDataset1dezembro: TfrxMemoView
          Left = 929.764380000000000000
          Width = 34.015770000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'dezembro'
          DataSet = frxDBDataset
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDataset1."dezembro"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxDBDataset1TotalAnual: TfrxMemoView
          Left = 963.780150000000000000
          Width = 83.149660000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Color = clBtnFace
          DataField = 'TotalAnual'
          DataSet = frxDBDataset
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDataset1."TotalAnual"]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object GroupHeader1: TfrxGroupHeader
        Height = 45.354360000000000000
        Top = 166.299320000000000000
        Width = 1046.929810000000000000
        Condition = 'frxDBDataset1."CodFamilia"'
        object frxDBDataset1CodFamilia: TfrxMemoView
          Width = 1046.929810000000000000
          Height = 26.456710000000000000
          ShowHint = False
          Color = 14211288
          DataSet = frxDBDataset
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8W = (
            
              ' Familia: [frxDBDataset1."CodFamilia"] - [frxDBDataset1."Sobreno' +
              'me"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo1: TfrxMemoView
          Left = 555.590910000000000000
          Top = 26.456710000000000000
          Width = 34.015748030000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'JAN')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo2: TfrxMemoView
          Left = 589.606680000000000000
          Top = 26.456710000000000000
          Width = 34.015748030000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'FEV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo3: TfrxMemoView
          Left = 623.622450000000000000
          Top = 26.456710000000000000
          Width = 34.015748030000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'MAR')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo4: TfrxMemoView
          Left = 657.638220000000000000
          Top = 26.456710000000000000
          Width = 34.015748030000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'ABR')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo5: TfrxMemoView
          Left = 691.653990000000000000
          Top = 26.456710000000000000
          Width = 34.015748030000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'MAI')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo6: TfrxMemoView
          Left = 725.669760000000000000
          Top = 26.456710000000000000
          Width = 34.015748030000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'JUN')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo7: TfrxMemoView
          Left = 759.685530000000000000
          Top = 26.456710000000000000
          Width = 34.015748030000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'JUL')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo10: TfrxMemoView
          Left = 793.701300000000000000
          Top = 26.456710000000000000
          Width = 34.015748030000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'AGO')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo11: TfrxMemoView
          Left = 827.717070000000000000
          Top = 26.456710000000000000
          Width = 34.015748030000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'SET')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo12: TfrxMemoView
          Left = 861.732840000000000000
          Top = 26.456710000000000000
          Width = 34.015748030000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'OUT')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo13: TfrxMemoView
          Left = 895.748610000000000000
          Top = 26.456710000000000000
          Width = 34.015748030000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'NOV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo14: TfrxMemoView
          Left = 929.764380000000000000
          Top = 26.456710000000000000
          Width = 34.015748030000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'DEZ')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo15: TfrxMemoView
          Top = 26.456710000000000000
          Width = 86.929168030000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8W = (
            '  Cod.')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo16: TfrxMemoView
          Left = 86.929190000000000000
          Top = 26.456710000000000000
          Width = 423.307338030000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8W = (
            ' Membro')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo17: TfrxMemoView
          Left = 510.236550000000000000
          Top = 26.456710000000000000
          Width = 45.354338030000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Ano')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo18: TfrxMemoView
          Left = 963.780150000000000000
          Top = 26.456710000000000000
          Width = 83.149638030000000000
          Height = 18.897650000000000000
          ShowHint = False
          Color = clBtnFace
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Total Anual')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object GroupFooter1: TfrxGroupFooter
        Height = 22.677180000000000000
        Top = 275.905690000000000000
        Width = 1046.929810000000000000
        object SysMemo3: TfrxSysMemoView
          Width = 1046.929810000000000000
          Height = 22.677180000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            
              'TOTAL GERAL DA FAM'#205'LIA: [SUM(<frxDBDataset1."TotalAnual">,Master' +
              'Data1)]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object ReportSummary1: TfrxReportSummary
        Height = 75.590600000000000000
        Top = 359.055350000000000000
        Width = 1046.929810000000000000
        object Memo19: TfrxMemoView
          Width = 1046.929810000000000000
          Height = 22.677180000000000000
          ShowHint = False
          Color = 14211288
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'TOTAIS:')
          ParentFont = False
          VAlign = vaCenter
        end
        object TotalFamilias: TfrxMemoView
          Left = 763.465060000000000000
          Top = 22.677180000000000000
          Width = 283.464750000000000000
          Height = 26.456710000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8W = (
            'Total de Fam'#237'lias: [TotalFamilias]')
          ParentFont = False
          VAlign = vaCenter
        end
        object TotalMembros: TfrxMemoView
          Left = 763.465060000000000000
          Top = 49.133890000000000000
          Width = 283.464750000000000000
          Height = 26.456710000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8W = (
            'Total de Membros: [TotalMembros]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
    end
  end
  object frxDBDataset: TfrxDBDataset
    UserName = 'frxDBDataset1'
    CloseDataSource = False
    DataSource = DSRel
    BCDToCurrency = False
    Left = 28
    Top = 152
  end
  object QryTotalMembros: TMyQuery
    Connection = DMPrincipal.Conn
    Left = 388
    Top = 68
  end
  object QryTotalFamilias: TMyQuery
    Connection = DMPrincipal.Conn
    Left = 388
    Top = 116
  end
end
