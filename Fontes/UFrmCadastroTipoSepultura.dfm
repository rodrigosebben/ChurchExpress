object FormCadastroTipoSepultura: TFormCadastroTipoSepultura
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = 'FormCadastroTipoSepultura'
  ClientHeight = 184
  ClientWidth = 453
  Color = clWindow
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  ShowHint = True
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel2: TPanel
    Left = 0
    Top = 0
    Width = 453
    Height = 49
    Align = alTop
    Caption = 'CADASTRO DE TIPO DE SEPULTURA'
    Color = clHotLight
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindow
    Font.Height = -21
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentBackground = False
    ParentFont = False
    TabOrder = 0
  end
  object Panel3: TPanel
    Left = 0
    Top = 135
    Width = 453
    Height = 49
    Align = alBottom
    Color = clHotLight
    ParentBackground = False
    TabOrder = 1
    object BtnSalvar: TBitBtn
      Left = 278
      Top = 5
      Width = 75
      Height = 41
      Caption = '&Salvar'
      Default = True
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      TabOrder = 0
      OnClick = BtnSalvarClick
    end
    object BtnCancelar: TBitBtn
      Left = 359
      Top = 5
      Width = 75
      Height = 41
      Caption = '&Cancelar'
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      TabOrder = 1
      OnClick = BtnCancelarClick
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 49
    Width = 453
    Height = 86
    Align = alClient
    Color = clInactiveCaption
    ParentBackground = False
    TabOrder = 2
    object Label1: TLabel
      Left = 16
      Top = 14
      Width = 113
      Height = 16
      Caption = 'Tipo de Sepultura'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object DBEdtNomeTipoSepultura: TDBEdit
      Left = 16
      Top = 36
      Width = 421
      Height = 24
      CharCase = ecUpperCase
      DataField = 'NomeTipoSepultura'
      DataSource = DSTipoSepultura
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
    end
  end
  object QryTipoSepultura: TMyQuery
    SQLInsert.Strings = (
      'INSERT INTO tiposepultura'
      '  (CodTipoSepultura, NomeTipoSepultura)'
      'VALUES'
      '  (:CodTipoSepultura, :NomeTipoSepultura)')
    SQLDelete.Strings = (
      'DELETE FROM tiposepultura'
      'WHERE'
      '  CodTipoSepultura = :Old_CodTipoSepultura')
    SQLUpdate.Strings = (
      'UPDATE tiposepultura'
      'SET'
      
        '  CodTipoSepultura = :CodTipoSepultura, NomeTipoSepultura = :Nom' +
        'eTipoSepultura'
      'WHERE'
      '  CodTipoSepultura = :Old_CodTipoSepultura')
    SQLRefresh.Strings = (
      'SELECT CodTipoSepultura, NomeTipoSepultura FROM tiposepultura'
      'WHERE'
      '  CodTipoSepultura = :CodTipoSepultura')
    SQLLock.Strings = (
      'SELECT * FROM tiposepultura'
      'WHERE'
      '  CodTipoSepultura = :Old_CodTipoSepultura'
      'FOR UPDATE')
    Connection = DMPrincipal.Conn
    SQL.Strings = (
      'SELECT * FROM tiposepultura'
      'order by nometiposepultura')
    Left = 400
    Top = 8
    object QryTipoSepulturaCodTipoSepultura: TLargeintField
      AutoGenerateValue = arAutoInc
      FieldName = 'CodTipoSepultura'
      Origin = 'tiposepultura.CodTipoSepultura'
    end
    object QryTipoSepulturaNomeTipoSepultura: TStringField
      FieldName = 'NomeTipoSepultura'
      Origin = 'tiposepultura.NomeTipoSepultura'
      Size = 200
    end
  end
  object DSTipoSepultura: TDataSource
    DataSet = QryTipoSepultura
    Left = 400
    Top = 56
  end
end
